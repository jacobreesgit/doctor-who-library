...
class UnifiedEnrichmentDisplay:
    ...
    def create_enrichment_table(self, items: List[Any], title: str = "Enrichment Status") -> Table:
...
🎯 [bold]Quality Metrics[/bold]
   Success Rate: {success_rate:.1f}%
        ...
        return Panel(stats_text.strip(), title="📊 Enrichment Statistics", border_style="green")
    ...
    try:
        ...
        with Progress(
            ...
            def enrichment_callback(item, processed, total):
                ...
                console.print(f"[{processed:04d}/{total:04d}] {status_icon} {item.title}")
                
                if item.enrichment_status.value == "enriched":
                    console.print(f"         📊 Confidence: {confidence:.2f} | 🔗 URL: {item.wiki_url or 'None'}")
        ...
        final_table = display.create_enrichment_table(
            all_items,
            f"Complete Final Status ({len(all_items)} items)"
        )
...
@click.command()
@click.option("--batch-size", default=10, help="Number of items to process in each batch")
@click.option("--max-items", default=None, type=int, help="Maximum number of items to process")
...
async def enrich(
    ...
    try:
        ...
        console.print(f"\n🔄 [bold]Starting enrichment of {pending_count} pending items...[/bold]")
        console.print("⚠️  [yellow]Note: This command shows the UI but enrichment functionality requires the EnrichmentService.[/yellow]")
        console.print("📝 [blue]Please run this command when the full enrichment system is properly wired.[/blue]")
