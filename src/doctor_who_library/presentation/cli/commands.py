...
async def enrich(
    ...
    try:
        ...
        console.print(
            f"📊 Current stats: {enrichment_stats.get('pending', 0)} pending, "
        ...
        if show_all:
            ...
            if processed_items:
                ...
                table = Table(
                    title=f"Complete Enrichment Status ({len(processed_items)} items)",
                    show_header=True,
                )
                ...
                for item in processed_items:
                    ...
                    confidence = (
                        f"{item.enrichment_confidence:.2f}"
                        if item.enrichment_confidence
                        else "N/A"
                    )
                    wiki_status = "✅" if item.wiki_url or item.wiki_summary else "❌"
                    error_note = (
                        (item.enrichment_error or "")[:25]
        ...
        with Progress(
            ...
            task = progress.add_task(
                "Enriching items...",
                total=min(pending_count, max_items) if max_items else pending_count,
            )
        ...
        if results["enriched"] > 0:
            console.print(
                f"📊 [bold]Avg Confidence:[/bold] {results['avg_confidence']:.2f}"
            )
...
def serve():
    ...
    try:
        ...
        raise click.ClickException("uvicorn not installed") from e
    except Exception as e:
...
def dev():
    ...
    try:
        ...
        console.print(
            "📊 [blue]Enrichment Monitor: Running with optimized display[/blue]"
        )
