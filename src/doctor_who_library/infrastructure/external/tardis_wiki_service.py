...
from doctor_who_library.domain.services.wiki_service import WikiService, WikiSearchResult
...
class TardisWikiService(WikiService):
    ...
    async def _search_for_item_internal(self, item: LibraryItem) -> Optional[WikiSearchResult]:
        ...
        for query in search_queries:
            try:
                ...
                for result in search_results:
                    ...
                    if page_content:
                        confidence = self._calculate_confidence_score(item, page_content)
        ...
        try:
            ...
            if result:
                ...
                item.mark_enrichment_skipped(
                    ...
                    search_term=item.get_search_titles()[0] if item.get_search_titles() else None,
                )
    ...
    async def enrich_items(self, items: List[LibraryItem]) -> List[LibraryItem]:
        ...
        semaphore = asyncio.Semaphore(self.config.max_concurrent if hasattr(self.config, 'max_concurrent') else 5)
        ...
        for i, result in enumerate(enriched_items):
            if isinstance(result, Exception):
                items[i].mark_enrichment_failed(
                    error=str(result),
                    search_term=items[i].get_search_titles()[0] if items[i].get_search_titles() else None,
                )
        ...
        try:
            ...
            if "query" in data and "search" in data["query"]:
                for item in data["query"]["search"]:
                    results.append({
                        "title": item["title"],
                        "url": urljoin(str(self.config.base_url), quote(item["title"].replace(" ", "_"))),
    ...
    async def _get_page_content(self, page_title: str) -> Optional[Dict[str, Any]]:
        """Get the content of a wiki page."""
        try:
            url = urljoin(str(self.config.base_url), quote(page_title.replace(" ", "_")))
            ...
            if content_div:
                ...
                for p in paragraphs:
                    ...
                    if text and len(text) > 50 and not text.startswith("{{") and not text.startswith("Fast Times"):
            ...
            content["images"] = [img.get("src") for img in images[:3]]
    ...
    def _calculate_confidence_score(self, item: LibraryItem, page_content: Dict[str, Any]) -> float:
        ...
        if item_title and page_title:
            ...
            elif any(word in page_title for word in item_title.split() if len(word) > 2):  # Lowered word length threshold
                score += 0.4
        
        # Very lenient Doctor Who relation check
        content_lower = (page_content.get("content", "") + " " + page_content.get("summary", "")).lower()
        ...
        doctor_who_terms = ["doctor", "tardis", "gallifrey", "dalek", "cybermen", "time lord", "bbc"]
        if any(term in content_lower or term in summary_lower for term in doctor_who_terms):
            score += 0.3
