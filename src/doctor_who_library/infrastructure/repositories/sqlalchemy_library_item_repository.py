...
from doctor_who_library.domain.repositories.library_item_repository import LibraryItemRepository
...
class SQLAlchemyLibraryItemRepository(LibraryItemRepository):
    ...
    async def save(self, item: LibraryItem) -> LibraryItem:
        ...
        try:
            async with self._session_factory() as session:
                ...
                existing_stmt = select(LibraryItemModel).where(LibraryItemModel.id == item.id)
    ...
    def _model_to_entity(self, model: LibraryItemModel) -> LibraryItem:
        ...
        try:
            ...
            return LibraryItem(
                ...
                serial_title=model.serial_title,
                content_type=self._safe_content_type(model.content_type),
    ...
    def _safe_enrichment_status(self, status_str: str) -> EnrichmentStatus:
        ...
        try:
            ...
            print(f"WARNING: Unknown enrichment status '{status_str}', defaulting to PENDING")
            return EnrichmentStatus.PENDING
    
    def _safe_content_type(self, content_type_str: str) -> Optional[ContentType]:
