...
class EnrichmentDisplayItem(BaseModel):
    ...
    activity_age_hours: float = Field(default=0.0, description="Hours since last activity")
    
    # Quality indicators
    confidence_level: str = Field(default="low", description="Human-readable confidence level")
    has_wiki_data: bool = Field(default=False, description="Whether wiki data was retrieved")
    ...
    def __post_init__(self):
        ...
        self.has_wiki_data = bool(self.wiki_url or self.wiki_summary or self.wiki_image_url)
...
class EnrichmentDisplayRequest(BaseModel):
    ...
    # Pagination
    limit: Optional[int] = Field(default=None, ge=1, description="Items per page (no limit if None)")
    cursor: Optional[str] = Field(default=None, description="Cursor for pagination")
    
    # Filtering
    status: Optional[List[EnrichmentActivityType]] = Field(default=None, description="Filter by status")
    section: Optional[List[str]] = Field(default=None, description="Filter by section")
    ...
    # Time filtering
    since: Optional[datetime] = Field(default=None, description="Items updated since this time")
    hours_back: Optional[int] = Field(default=None, ge=1, le=8760, description="Hours to look back")
    ...
    # Display options
    include_stats: bool = Field(default=True, description="Include statistics in response")
    include_pending: bool = Field(default=False, description="Include pending items")
    mark_recent: bool = Field(default=True, description="Mark items as recent based on activity")
    recent_hours: int = Field(default=2, ge=1, le=24, description="Hours to consider as recent")
...
class EnrichmentStats(BaseModel):
    ...
    high_confidence_count: int = Field(default=0, description="Items with confidence >= 0.8")
    medium_confidence_count: int = Field(default=0, description="Items with confidence >= 0.6")
    low_confidence_count: int = Field(default=0, description="Items with confidence < 0.6")
...
class EnrichmentProgressUpdate(BaseModel):
    ...
    # Update type
    update_type: str = Field(description="Type of update (item_processed, batch_complete, etc.)")
    ...
    # Processing result
    status: Optional[EnrichmentActivityType] = None
    ...
    # Performance metrics
    processing_time: Optional[float] = Field(default=None, description="Processing time for this item")
